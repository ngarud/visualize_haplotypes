...
# This definition runs all other definitions to identify haplotypes, their clusters, and summary statistics. This is run for every analysis window. 
        ...
        # UPGMA_clustering
        [keyVector, sizeVector] = UPGMA_clusters(haps_clumped)

        # find all clusters with at least two haplotypes
        ...
        printClusters(inFile, outFile, centerCoord, haps_clumped,  keyVector, sizeVector, absLengthWin, edgeCoord1, edgeCoord2)
...
###################
def UPGMA_clusters(haps_clumped):
    # this definition first computes all the distances between each cluster.
    # then it calls the UPGMA.py script to make a phylogenetic tree out of this
    # then it orders the clusters based on this and returns the keys in this order.
    # this is how the clusters will be printed. 
    
    # need to create a matrix of number of unique haps, lower diag
    # initiatlize a new numpy array:
    unique_haplotypes=haps_clumped.keys()
    num_unique_haps=len(unique_haplotypes)
    distance_matrix = numpy.zeros(shape=(num_unique_haps,num_unique_haps))  


    i=0
    for haplotype1 in unique_haplotypes:
        j=0
        for haplotype2 in unique_haplotypes:
            [distance, s1]= hamming_distance_clump(haplotype1, haplotype2, int(windowTot))
            distance_matrix[i,j] = distance
            j+=1
        i+=1
            

    # get the indices of the lower diagonal:
    idxs=numpy.tril_indices(num_unique_haps,-1)
    
    # get the entries of the lower diagonal:
    lower_diag_entries=distance_matrix[idxs].tolist()

    # create a list of lists 
    lower_diag_entries_lists=[[]]
    end=1
    while len(lower_diag_entries) >0:
        new_list=[]
        for i in range(0, end):
            new_list.append(lower_diag_entries.pop(0))
        end +=1
        lower_diag_entries_lists.append(new_list)
    ...
    # now call the UPGMA algorithm
    UPGMA_order=UPGMA.UPGMA(lower_diag_entries_lists, labels)

    # create a key_vector and a size_vector using the UPGMA_order
    
    # first decompose UPGMA_order into a list
    UPGMA_order_list=[]
    numbers_unparsed=re.split('[( ) , ]',UPGMA_order)
    for number_unparsed in numbers_unparsed:
        if number_unparsed != '':
                UPGMA_order_list.append(int(number_unparsed))
    #for i in range(0, len(UPGMA_order)):
    #    if UPGMA_order[i] != '(' and UPGMA_order[i] != ')' and UPGMA_order[i] != ',':
    #        UPGMA_order_list.append(int(UPGMA_order[i]))

    keyVector=[]
    sizeVector=[]

    for i in UPGMA_order_list:
        keyVector.append(unique_haplotypes[i])
        sizeVector.append(len(haps_clumped[unique_haplotypes[i]]))
...
######################
def printClusters(inFile, outFile, centerCoord, haps,  keyVector, sizeVector, absLengthWin, edgeCoord1, edgeCoord2):
